...
import static org.geotools.process.opensearch.OpenSearchBucketVisitor.ES_AGGREGATE_BUCKET;
...
import org.geotools.data.opensearch.date.DateFormat;
...
/**
 * Encodes an OGC {@link Filter} and creates a filter for an OpenSearch query. Optionally applies
 * SQL View parameters from {@link Query} defining OpenSearch query directly.
 *
 * <p>Based on org.geotools.data.jdbc.FilterToSQL in the GeoTools library/jdbc module.
 */
class FilterToOpenSearch implements FilterVisitor, ExpressionVisitor {

    /** Standard java logger */
    static final Logger LOGGER = Logging.getLogger(FilterToOpenSearch.class);
    ...
    private final FilterToOpenSearchHelper helper;
    ...
    public FilterToOpenSearch() {
        queryBuilder = OpenSearchConstants.MATCH_ALL;
        nativeQueryBuilder = ImmutableMap.of("match_all", Collections.emptyMap());
        helper = new FilterToOpenSearchHelper(this);
    }
     ...
     * @throws FilterToOpenSearchException If there were io problems.
     */
    public void encode(Filter filter) throws FilterToOpenSearchException {
     ...
     * @throws FilterToOpenSearchException If there were io problems.
     */
    public void encode(Query query) throws FilterToOpenSearchException {
    ...
    Capabilities createCapabilities() {
        return new OpenSearchCapabilities();
    }
    ...
    public Object visit(ExcludeFilter filter, Object extraData) {
        queryBuilder =
                ImmutableMap.of("bool", ImmutableMap.of("must_not", OpenSearchConstants.MATCH_ALL));
        return extraData;
    }
    ...
    public Object visit(PropertyIsBetween filter, Object extraData) {
        ...
        if (attType != null) {
            ...
            if (attType.getUserData().containsKey(OpenSearchConstants.NESTED)) {
                nested = (Boolean) attType.getUserData().get(OpenSearchConstants.NESTED);
            }
    ...
    public Object visit(PropertyIsLike filter, Object extraData) {
        ...
        nested = false;
        if (attType != null) {
            if (attType.getUserData().containsKey(OpenSearchConstants.ANALYZED)) {
                analyzed = (Boolean) attType.getUserData().get(OpenSearchConstants.ANALYZED);
            }
            if (attType.getUserData().containsKey(OpenSearchConstants.NESTED)) {
                nested = (Boolean) attType.getUserData().get(OpenSearchConstants.NESTED);
            }
     ...
     */
    @Override
    public Object visit(Literal expression, Object context) throws FilterToOpenSearchException {
        ...
        try {
            ...
            throw new FilterToOpenSearchException("IO problems writing literal", e);
        }
        return context;
    }
    ...
    private void updateDateFormatter(AttributeDescriptor attType) {
        if (attType != null) {
            ...
            final List<String> validFormats =
                    (List<String>) attType.getUserData().get(OpenSearchConstants.DATE_FORMAT);
    ...
    void addViewParams(Query query) {
        ...
        if (hints != null && hints.get(ES_AGGREGATE_BUCKET) != null) {
            ...
            if (nativeOnly) {
                LOGGER.fine("Ignoring GeoServer filter (OpenSearch native query/post filter only)");
                queryBuilder = OpenSearchConstants.MATCH_ALL;
            }
    ...
    private void setupNativeQuery(String nativeQuery) {
        try {
            ...
            try {
                nativeQueryBuilder =
                        mapReader.readValue(OpenSearchParserUtil.urlDecode(nativeQuery));
            } catch (Exception e2) {
                throw new FilterToOpenSearchException("Unable to parse native query", e);
            }
        }
    }
    ...
    private void setGeometryField(Map<String, Object> m) {
        if ("".equals(m.get("field"))) {
            ...
            String name = (String) gd.getUserData().get(OpenSearchConstants.FULL_NAME);
    ...
    public Map<String, Object> getQueryBuilder() {
        ...
        if (nativeQueryBuilder.equals(OpenSearchConstants.MATCH_ALL)) {
            queryBuilder = this.queryBuilder;
        } else if (this.queryBuilder.equals(OpenSearchConstants.MATCH_ALL)) {
