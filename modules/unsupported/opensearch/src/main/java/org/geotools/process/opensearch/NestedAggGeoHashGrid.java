 ...
 */
package org.geotools.process.opensearch;
...
public class NestedAggGeoHashGrid extends GeoHashGrid {
    ...
    public void setParams(List<String> params) {
        if (null != params) {
            if (params.size() < 5) {
                LOGGER.warning(
                        "Parameters list does not contain required length; you provided "
                                + params.size()
                                + ", expecting: 5 or more");
            ...
            switch (params.get(SELECTION_STRATEGY_INDEX)) {
                ...
                default:
                    LOGGER.warning(
                            "Unexpected buckets selection strategy parameter; you provided "
            ...
            switch (params.get(RASTER_STRATEGY_INDEX)) {
                ...
                default:
                    LOGGER.warning(
                            "Unexpected raster strategy parameter; you provided "
            ...
            if (rasterStrategy.equals(RASTER_FROM_KEY) && params.size() >= 6) {
                ...
                for (String term : terms) {
                    ...
                    if (keyValueSplit.length != 2) {
                        LOGGER.warning(
                                "Term " + term + " does not contain required format <key>:<value>");
                        throw new IllegalArgumentException();
                    }
        ...
        if (rasterStrategy.equals(RASTER_FROM_KEY)) {
            if (null != termsMap) {
                if (termsMap.containsKey(key)) {
                    ...
                    LOGGER.warning(
                            "Cannot convert key (String) to raster value, mapping does not contain key "
                                    + key
                ...
                try {
                    ...
                    LOGGER.warning(
                            "Cannot convert key (String) to raster value, key, "
                                    + key
