...
import static org.geotools.data.opensearch.OpenSearchConstants.MATCH_ALL;
...
class FilterToOpenSearchHelper {
    ...
    private final FilterToOpenSearch delegate;

    public FilterToOpenSearchHelper(FilterToOpenSearch delegate) {
    ...
    private void visitBinarySpatialOperator(
        ...
        OpenSearchGeometryType geometryType =
                (OpenSearchGeometryType) attType.getUserData().get(GEOMETRY_TYPE);
        if (geometryType == OpenSearchGeometryType.GEO_POINT) {
    ...
    private void visitGeoShapeBinarySpatialOperator(
        ...
        if (filter instanceof Disjoint) {
            ...
            FilterToOpenSearch.LOGGER.fine(
