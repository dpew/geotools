...
/** FeatureReader access to the Opensearch index. */
class OpenSearchFeatureReader implements FeatureReader<SimpleFeatureType, SimpleFeature> {

    private static final Logger LOGGER = Logging.getLogger(OpenSearchFeatureReader.class);
    ...
    private final OpenSearchDataStore.ArrayEncoding arrayEncoding;

    private SimpleFeatureBuilder builder;

    private Iterator<OpenSearchHit> searchHitIterator;

    private Iterator<Map<String, Object>> aggregationIterator;

    private final OpenSearchParserUtil parserUtil;

    public OpenSearchFeatureReader(ContentState contentState, OpenSearchResponse response) {
        this(contentState, response.getHits(), response.getAggregations(), response.getMaxScore());
    }

    public OpenSearchFeatureReader(
            ContentState contentState,
            List<OpenSearchHit> hits,
            Map<String, OpenSearchAggregation> aggregations,
            float maxScore) {
        ...
        this.parserUtil = new OpenSearchParserUtil();
        ...
        if (contentState.getEntry() != null && contentState.getEntry().getDataStore() != null) {
            final OpenSearchDataStore dataStore =
                    (OpenSearchDataStore) contentState.getEntry().getDataStore();
            this.arrayEncoding = dataStore.getArrayEncoding();
        } else {
            this.arrayEncoding =
                    OpenSearchDataStore.ArrayEncoding.valueOf(
                            (String) OpenSearchDataStoreFactory.ARRAY_ENCODING.getDefaultValue());
        }
    ...
    private String nextHit() {
        final OpenSearchHit hit = searchHitIterator.next();
        ...
        for (final AttributeDescriptor descriptor : type.getAttributeDescriptors()) {
            ...
            final String sourceName =
                    (String) descriptor.getUserData().get(OpenSearchConstants.FULL_NAME);
            ...
            if (values == null && sourceName.equals("_id")) {
                ...
                if (dataVal instanceof Double) {
                    ...
                    final List<String> validFormats =
                            (List<String>)
                                    descriptor.getUserData().get(OpenSearchConstants.DATE_FORMAT);
                ...
                final Object value;
                if (arrayEncoding == OpenSearchDataStore.ArrayEncoding.CSV) {
