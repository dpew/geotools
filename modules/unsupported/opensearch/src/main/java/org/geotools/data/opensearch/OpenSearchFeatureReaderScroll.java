...
class OpenSearchFeatureReaderScroll implements FeatureReader<SimpleFeatureType, SimpleFeature> {

    private static final Logger LOGGER = Logging.getLogger(OpenSearchFeatureReaderScroll.class);
    ...
    private OpenSearchFeatureReader delegate;
    ...
    public OpenSearchFeatureReaderScroll(
            ContentState contentState, OpenSearchResponse searchResponse, int maxFeatures) {
    ...
    private void processResponse(OpenSearchResponse searchResponse) {
        final int numHits = searchResponse.getNumHits();
        final List<OpenSearchHit> hits;
        ...
        delegate =
                new OpenSearchFeatureReader(
                        contentState, hits, searchResponse.getAggregations(), 0);
