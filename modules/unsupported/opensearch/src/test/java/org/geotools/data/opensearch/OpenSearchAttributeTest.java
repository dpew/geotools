...
public class OpenSearchAttributeTest {

    private OpenSearchAttribute attr;
    ...
    private OpenSearchAttribute.OpenSearchGeometryType geometryType;
    ...
    public void setup() {
        name = "theName";
        attr = new OpenSearchAttribute(name);
        customName = "XML Custom Name";
        normalizedName = "_XML_Custom_Name";
        type = Map.class;
        geometryType = OpenSearchAttribute.OpenSearchGeometryType.GEO_SHAPE;
        use = true;
    ...
    public void testHashCode() {
        assertEquals(attr.hashCode(), (new OpenSearchAttribute("theName")).hashCode());
        assertNotEquals(attr.hashCode(), (new OpenSearchAttribute("name")).hashCode());
    }

    @Test
    public void testEquals() {
        assertEquals(attr, new OpenSearchAttribute("theName"));
        assertNotEquals(attr, new OpenSearchAttribute("name"));
    }

    @Test
    public void testClone() {
        assertEquals(attr, new OpenSearchAttribute(attr));
    }

    @Test
    public void testCompare() {
        OpenSearchAttribute other = new OpenSearchAttribute("other");
        attr.setOrder(1);
        other.setOrder(2);
        ...
        other.setOrder(null);
        assertEquals(-1, attr.compareTo(other));
        other = new OpenSearchAttribute("zAfter");
        attr.setOrder(null);
        other.setOrder(null);
        assertTrue(attr.compareTo(other) < 0);
        other = new OpenSearchAttribute("before");
        attr.setOrder(1);
        other.setOrder(1);
        assertTrue(attr.compareTo(other) > 0);
    }
