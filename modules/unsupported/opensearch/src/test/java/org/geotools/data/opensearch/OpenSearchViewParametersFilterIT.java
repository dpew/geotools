...
public class OpenSearchViewParametersFilterIT extends OpenSearchTestSupport {
    ...
    public void testNativeTermQuery() throws Exception {
        ...
        List<Hints.ClassKey> keys =
                Arrays.asList(
                        ...
                        OpenSearchBucketVisitor.ES_AGGREGATE_BUCKET);
    ...
    public void testNativeAggregation() throws Exception {
        init();
        Query q = new Query(featureSource.getSchema().getTypeName());
        OpenSearchBucketVisitor bucketVisitor =
                new OpenSearchBucketVisitor(
                        "{\"agg\": {\"geohash_grid\": {\"field\": \"geo\", \"precision\": 3}}}",
                        null);
        featureSource.accepts(q, bucketVisitor, null);
        List<Map<String, Object>> buckets = bucketVisitor.getBuckets();
