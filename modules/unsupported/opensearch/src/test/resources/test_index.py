...
requests.delete(f'{opensearch_url}/{index_name}', auth=auth)
requests.put(f'{opensearch_url}/{index_name}', auth=auth)
...
requests.put(f'{opensearch_url}/{index_name}/_mapping/{type_name if version < 7 else ""}', json=mappings, auth=auth)
...
for item in [item for item in features if item['status_s'] == 'active']:
    ...
    requests.put(f'{opensearch_url}/{index_name}/{type_name}/{item["id"]}', json=item, auth=auth)

requests.put(f'{opensearch_url}//_xpack/security/role/status_admin', json={'indices': [{'privileges': ['all'], 'names': ['status*']}]}, auth=auth)
requests.put(f'{opensearch_url}/_xpack/security/user/admin', json={'password': 'statusadmin', 'roles': ['status_admin']}, auth=auth)
